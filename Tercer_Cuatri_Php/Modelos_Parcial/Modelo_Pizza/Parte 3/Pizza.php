...
//        echo "Entre 3";
    ...
    //retorna , 0 si se pudo vender, restar y actualizar json, -1 si dato vino malo, -2 si no existe el producto
        //-3 si no hay suficiente stock
    public static function generarVenta($pedidoPizza)
    {
        if(!is_null($pedidoPizza) && get_class($pedidoPizza) == "Pizza")
        {
            $arrayPizzas = Pizza::retornarArrayPizza_Json();
            $index = $pedidoPizza->estaEnArray($arrayPizzas);
            if($index > -1)
            {
                $cantidadExistente = $arrayPizzas[$index]->getCantidad();
                $cantidadFinal = $cantidadExistente - $pedidoPizza->getCantidad();
                echo "Cantidad anterior: " . $cantidadExistente . "\n\n";
                echo "Cantidad a restar: " . $pedidoPizza->getCantidad() . "\n\n";
                echo "Resultado es: " . $cantidadFinal . "\n\n";
                if($cantidadFinal > -1)
                {
                    $arrayPizzas[$index]->setCantidad($cantidadFinal);
                    if(Pizza::guardarArrayPizza_Json($arrayPizzas))
                    {
                        return 0; //se puedo vender
                    }
                }
                else
                {
            ...
            {
                return -2; //no existe ese producto
            }
        }
        return -1; //datos invalidos
    }
}
