...
class Venta
{
    ...
    public function __construct($id = -1, $email,$fecha = "1900-01-01",$sabor,$tipo,$cantidad)
    {
        $this->setId($id);
        $this->setEmail($email);
        $this->setFecha($fecha);
    ...
    public static function encontrarVenta_BD($id)
    {
        $venta = null;
        $objPDO = AccesoDatos::dameUnObjetoAcceso();
        if(!is_null($objPDO))
        {
            $query = $objPDO->RetornarConsulta("SELECT * FROM ventas_pizzas WHERE id = :id");
    ...
    {
        if(!is_null($venta) && get_class($venta) === "Venta")
        {
            if(!is_null(Venta::encontrarVenta_BD($venta->getId())))
            {
                $objPDO = AccesoDatos::dameUnObjetoAcceso();
                if(!is_null($objPDO))
                {//email sabor tipo cantidad
                    $query = $objPDO->RetornarConsulta("UPDATE ventas_pizzas SET email = :mail , sabor = :sabor , tipo = :tipo , cantidad = :cant 
                                                                                WHERE id = :id");
                    $query->bindValue(":mail",$venta->getEmail());
                    $query->bindValue(":sabor",$venta->getSaborVenta());
                    $query->bindValue(":tipo",$venta->getTipoVenta());
                    $query->bindValue(":cant",$venta->getCantidadVenta());
                    $query->bindValue(":id",$venta->getId());
                    $query->execute();

                    if($query->rowCount() > 0)
                    {
                        return 0; //se pudo actualizar
                    }
                    else
                    {
                        return -3; //por x motivo no se pudo actualizar
                    }
                }
            }
            return -2;//no existe la venta en el listado 
        }
        return -1; //algun dato vino erroneo
    } 

    public static function borrarRegistroVenta_BD($id)
    {
        if(!is_null($id) && is_numeric($id))
        {
            if(!is_null(Venta::encontrarVenta_BD($id)))
            {
                $objPDO = AccesoDatos::dameUnObjetoAcceso();
                if(!is_null($objPDO))
                {//email sabor tipo cantidad
                    $query = $objPDO->RetornarConsulta("DELETE FROM ventas_pizzas WHERE id = :id");
                    $query->bindValue(":id",$id);
                    $query->execute();
                    if($query->rowCount() > 0)
                    {
                        return 0; //se pudo borrar
                    }
                    else
                    {
                        return -3; //por x motivo no se pudo borrar
                    }
                }
            }
            return -2;//no existe la venta en el listado 
